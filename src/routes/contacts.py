...
async def show_contacts(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    contacts = await repository_contacts.show_contacts(skip, limit, db)
    return contacts
...
async def search_contacts(credentials: str, db: Session = Depends(get_db)):
    contact = await repository_contacts.search_contacts(credentials, db)
    if contact is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Contact not found")
    return contact
...
async def upcoming_birthday(db: Session = Depends(get_db)):
    contact = await repository_contacts.upcoming_birthday(db)
    if contact is None:
